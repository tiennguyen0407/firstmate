    ...
    if text.startswith("/follow"):
        ...
        if not action:
            await get_bot().send_message(
                ...
                "_Manager sẽ dùng context từ task trước để điền service nếu thiếu._",
                parse_mode="Markdown",
            )
            return {"ok": True}
        ...
        asyncio.create_task(_safe_task(_maybe_clarify_action(chat_id, action, requester_name), chat_id))
        return {"ok": True}
...
async def _route_message(chat_id: str, text: str, name: str):
    ...
    if msg_type is None:
        ...
        try:
            msg_type = (await _classify_message_type(text)).get("type", "action")
        except Exception as exc:
            error_log.append({"error": f"{type(exc).__name__}: {exc}", "source": "classify_message_type"})
            logger.error(f"classify_message_type error: {exc}")
            msg_type = "action"

    logger.info(f"route chat={chat_id} status={conv.get('status')} type={msg_type} quick={_quick_classify(text)} text={text[:60]!r}")

    # Knowledge → manager trả lời luôn, không cần SRE
    if msg_type == "knowledge":
        try:
            answer = await _answer_knowledge_question(text)
    ...
    # Action + conv active → forward SRE như follow-up
    if conv.get("status") != "completed":
        await _handle_conversation_reply(chat_id, text, name)
        return

    # Action + conv completed → classify new_task vs followup
    try:
    ...
    if chat_type == "new_task":
        del _active_conv[chat_id]
        await _maybe_clarify_action(chat_id, text, name)
    else:
        await _handle_conversation_reply(chat_id, text, name)


async def _start_new_task(chat_id: str, text: str, requester_name: str):
    """Tạo job mới (đã thoát conv hoặc chưa có conv)."""
    service = _detect_service(text)
    job_id = str(uuid.uuid4())

    if DEBUG_MODE:
        logger.info(f"[DEBUG] new job={job_id[:8]} service={service} from={chat_id}")
        ...
        asyncio.create_task(
            _debug_forward_to_sre(job_id, text, chat_id, requester_name, service)
        )
    else:
...
async def _handle_callback(query: CallbackQuery):
    try:
        ...
        # ── Claude done verify ────────────────────────────────────
            ...
            if action == "done":
                if requester:
                    summary_truncated = summary[:800] + ("..." if len(summary) > 800 else "")
                    html_summary = _md_to_html(summary_truncated)
                    await get_bot().send_message(
                        requester,
                        f"✅ <b>Kết quả điều tra từ FirstMate</b>\n\n{html_summary}\n\n"
                        f"Có yêu cầu tiếp theo? Nhắn thẳng vào đây hoặc dùng /follow &lt;action&gt;.",
                        parse_mode="HTML",
                    )
...
_KNOWLEDGE_RE = _re.compile(
    r'(là gì|là cái gì|nghĩa là gì|là server gì|là node gì|là pod gì|là namespace gì'
    r'|là subnet gì|là service gì|là ip gì|là địa chỉ gì'
    r'|hoạt động (như thế nào|ra sao|thế nào)'
    r'|dùng để (làm gì|làm việc gì)'
    r'|khác .{1,30} (thế nào|như thế nào|ra sao)'
...
_ACTION_RE = _re.compile(
    ...
    r'|không (chạy|start|up|respond|healthy))\b'
    r'|^(check|kiểm tra) \S',
    _re.IGNORECASE,
)
...
def _quick_classify(text: str) -> str | None:
    """Regex pre-classifier — bỏ qua LLM call khi pattern rõ ràng.

    Returns 'knowledge' | 'action' | None (ambiguous → cần LLM).
    """
    t = text.strip()
...
async def _classify_message_type(text: str) -> dict:
    """Phân loại tin nhắn: 'knowledge' (câu hỏi/giải thích) hay 'action' (cần hệ thống/SRE).

    knowledge: câu hỏi khái niệm, giải thích, how-to — không cần truy cập k8s.
    action: check pod, scale, restart, deploy, xem log, điều tra lỗi — cần SRE/runner.
    """
    from langchain_openai import ChatOpenAI
    from langchain_core.messages import SystemMessage, HumanMessage
    ...
    system = """/no_think
Phân loại tin nhắn từ developer/QC trong hệ thống DevOps:
- "knowledge": câu hỏi có thể trả lời mà KHÔNG cần chạy lệnh hay truy cập live system
  Gồm: giải thích khái niệm, hỏi thông tin hạ tầng (IP, server, namespace, service, team, network)
  VD: "statefulset là gì", "10.40.81.2 là server gì", "namespace zpp-loyalty-qc dùng cho gì",
      "loyalty-reward-store service làm gì", "team loyalty ai lead", "pod khác deployment thế nào"
- "action": yêu cầu kiểm tra hoặc thay đổi trạng thái LIVE system — cần chạy kubectl/bash
  VD: "check pod loyalty", "scale lên 10", "restart service", "xem log hiện tại", "service đang lỗi",
      "số pod đang chạy là bao nhiêu", "memory usage của pod"

Trả về JSON: {"type": "knowledge"} hoặc {"type": "action"}"""
    ...
    try:
        ...
        return {"type": "action"}  # safe default


async def _answer_knowledge_question(text: str) -> str:
    """Trả lời câu hỏi kiến thức DevOps/K8s, có inject System Knowledge Base."""
    from langchain_openai import ChatOpenAI
    from langchain_core.messages import SystemMessage, HumanMessage
    ...
    system = "/no_think\nBạn là FirstMate — AI assistant của team DevOps/SRE.\n" \
             "Trả lời câu hỏi về Kubernetes, DevOps, hệ thống phân tán một cách ngắn gọn và thực tế.\n" \
             "Dùng tiếng Việt. Dùng Markdown để format (bold, bullet points) nếu giúp dễ đọc hơn.\n" \
             "Không cần nói \"Tôi là AI\" hay disclaimer — trả lời thẳng vào câu hỏi.\n\n" \
             "Khi câu hỏi liên quan đến IP, server, service, namespace của hệ thống, " \
             "ưu tiên dùng thông tin trong Knowledge Base bên dưới.\n\n"
    if kb_context:
        system += kb_context
    ...
    if not answer:
        # Qwen3 thinking mode có thể trả về content rỗng khi hết token budget
        answer = (response.additional_kwargs.get("reasoning_content") or "").strip()
    if not answer:
        raise ValueError("LLM returned empty response — có thể do thinking mode hoặc token limit")
    return answer


async def _classify_and_respond(chat_id: str, text: str, requester_name: str):
    """Phân loại tin nhắn mới: câu hỏi kiến thức → trả lời luôn, action → forward SRE."""
    bot = get_bot()
    ...
    logger.info(f"classify_and_respond chat={chat_id} type={msg_type} quick={_quick_classify(text)} text={text[:60]!r}")
    ...
    if msg_type == "knowledge":
        ...
        await _maybe_clarify_action(chat_id, text, requester_name)
...
async def _maybe_clarify_action(chat_id: str, text: str, name: str):
    ...
    if service != "unknown" or not history:
        await _start_new_task(chat_id, text, name)
        return

    # Vague + có history → LLM suy luận context
    try:
    ...
    if result.get("ready"):
        await _start_new_task(chat_id, result["action"], name)
    else:
        # Lưu pending state và hỏi xác nhận
        _active_conv[chat_id] = {
...
async def _handle_pending_confirm(chat_id: str, text: str, name: str, conv: dict):
    ...
    if text_lower in _YES_WORDS or any(w in text_lower for w in _YES_WORDS):
        ...
        del _active_conv[chat_id]
        await _start_new_task(chat_id, confirmed, name)
        ...
        # Dev/QC cung cấp thêm info / sửa lại → xử lý như action mới
        del _active_conv[chat_id]
        await _maybe_clarify_action(chat_id, text, name)
...
async def job_complete(payload: _ClaudeDonePayload):
    ...
    # Truncate trước khi convert để HTML tags luôn balanced
    summary_truncated = payload.summary[:800] + ("..." if len(payload.summary) > 800 else "")
    html_preview = _md_to_html(summary_truncated)
    msg = (
        f"🤖 <b>FirstMate đã điều tra xong!</b>\n\n"
        f"📊 <b>Kết quả:</b>\n{html_preview}\n\n"
        f"Vui lòng verify và xác nhận:"
    )
    keyboard = InlineKeyboardMarkup([[
    ...
    try:
        await get_bot().send_message(sre_chat, msg, reply_markup=keyboard, parse_mode="HTML")
    except Exception as exc:
...
def _detect_service(text: str) -> str:
    ...
    # Fallback cuối: từ đầu tiên có nghĩa (>3 ký tự)
    words = [w for w in re.findall(r'\b[a-zA-Z]{4,}\b', text) if w.lower() not in
             {"check", "kiểm", "xem", "giúp", "tôi", "bao", "nhiêu", "đang", "service"}]
